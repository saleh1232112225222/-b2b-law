...
const PATTERNS = {
  ...
  vcard: {
    ...
    check: (content) => {
      ...
      return lines.some(
        (line) =>
          /<v-card[^>]*>/.test(line) && !/glass-card/.test(line) && !/v-data-table/.test(line)
      )
    }
  },
  ...
  input: {
    match:
      /<(v-text-field|v-select|v-autocomplete|v-textarea|v-combobox)[\s>](?![\s\S]*?class=["'][^"']*glass-input[^"']*["'])/g
  },

  // 3. v-btn رئيسي (color="accent" أو color="primary" أو اللون الذهبي) بدون premium-btn-gold-gradient
  btn: {
    match:
      /<v-btn[\s>](?=[\s\S]*?color=["'](?:accent|primary|gold)["'])(?![\s\S]*?class=["'][^"']*premium-btn-gold-gradient[^"']*["'])[\s\S]*?>/g
  }
}
...
function enhanceVCard(content) {
  ...
  content = content.replace(/<v-card([\s>])(?![\s\S]*?class=)/g, '<v-card$1 class="glass-card" ')
...
function enhanceButtons(content) {
  ...
  content = content.replace(
    ...
    (match, before, clsOpen, clsVal, clsClose) => {
      ...
      const newClass = clsVal.trim()
        ? `${clsVal.trim()} premium-btn-gold-gradient`
  ...
  content = content.replace(
    ...
    (match, before, clsOpen, clsVal, clsClose) => {
      if (clsVal.includes('premium-btn-gold-gradient') || clsVal.includes('v-data-table'))
        return match
      const newClass = clsVal.trim()
        ? `${clsVal.trim()} premium-btn-gold-gradient`
        : 'premium-btn-gold-gradient'
...
function enhanceDialogs(content) {
  ...
  content = content.replace(/<v-card[\s>][\s\S]*?(?=<\/v-card>[\s\S]*?<\/v-dialog>)/g, (match) => {
