 ...
 */
const { analyzeJudgment, classifyOutcome, determineDegree } = require('./cloud-server/dist/services/judgmentAnalyzer.service.js');
...
test('تم إنشاء مهام', () => assert(s2.tasks.length >= 2, `Expected >=2 tasks, got ${s2.tasks.length}`));

const s2Titles = s2.tasks.map(t => t.title);
test('مهمة: تقديم طلب النقض أو ما يشابهها', () => assert(s2Titles.some(t => t.includes('تقديم') || t.includes('نقض')), JSON.stringify(s2Titles)));
test('مهمة: دراسة أسباب الاعتراض', () => assert(s2Titles.some(t => t.includes('دراسة')), JSON.stringify(s2Titles)));
test('مهمة: تبليغ العميل بالنتيجة والمخاطر', () => assert(s2Titles.some(t => t.includes('المخاطر')), JSON.stringify(s2Titles)));
test('مدة الاعتراض = 30 يوم (تجارية)', () => assert(s2.deadlines?.appealDeadlineDays === 30, `Expected 30, got ${s2.deadlines?.appealDeadlineDays}`));
...
test('مهمة: تبليغ العميل بالنتيجة', () => assert(s3Titles.some(t => t.includes('تبليغ العميل')), JSON.stringify(s3Titles)));
test('مهمة: أرشفة القضية', () => assert(s3Titles.some(t => t.includes('أرشفة')), JSON.stringify(s3Titles)));

// ──── Additional Tests ────
console.log('\n📋 اختبارات إضافية: المدد القانونية\n');

test('مدنية = 30 يوم', () => assert(
  analyzeJudgment({ result: 'صدور حكم', caseType: 'مدنية', isForClient: false, hasAppealGrounds: true }).deadlines?.appealDeadlineDays === 30
));
test('تجارية = 30 يوم', () => assert(
  analyzeJudgment({ result: 'صدور حكم', caseType: 'تجارية', isForClient: false, hasAppealGrounds: true }).deadlines?.appealDeadlineDays === 30
));
test('جنائية = 30 يوم', () => assert(
  analyzeJudgment({ result: 'صدور حكم', caseType: 'جنائية', isForClient: false, hasAppealGrounds: true }).deadlines?.appealDeadlineDays === 30
));
test('عمالية = 30 يوم', () => assert(
  analyzeJudgment({ result: 'صدور حكم', caseType: 'عمالية', isForClient: false, hasAppealGrounds: true }).deadlines?.appealDeadlineDays === 30
));
test('إدارية = 60 يوم', () => assert(
  analyzeJudgment({ result: 'صدور حكم', caseType: 'إدارية', isForClient: false, hasAppealGrounds: true }).deadlines?.appealDeadlineDays === 60
));
...
const h1 = analyzeJudgment({ result: 'حجز القضية للحكم', judgmentType: 'ابتدائي', isForClient: true, needsExecution: false, hasAppealGrounds: false });
const h1Titles = h1.tasks.map(t => t.title);
test('مهمة: متابعة تاريخ النطق بالحكم', () => assert(h1Titles.some(t => t.includes('النطق بالحكم')), JSON.stringify(h1Titles)));

console.log('\n📋 اختبارات: مهام تأجيل\n');
const a1 = analyzeJudgment({ result: 'تأجيل الجلسة لموعد آخر', judgmentType: 'ابتدائي', isForClient: true, needsExecution: false, hasAppealGrounds: false });
const a1Titles = a1.tasks.map(t => t.title);
test('مهمة: متابعة تاريخ الجلسة الجديدة', () => assert(a1Titles.some(t => t.includes('الجلسة الجديدة')), JSON.stringify(a1Titles)));
...
for (const t of tablesToCheck) {
  test(`جدول موجود: ${t}`, () => assert(schema.includes(`CREATE TABLE ${t}`), `Table ${t} not found in schema`));
}
