...
function check(ok, msg, detail = '') {
  ...
  else { console.log(`  ❌ ${msg} ${detail}`); fail++; }
}
...
const requiredTables = [
  'companies', 'clients', 'defendants', 'employees', 'users',
  'cases', 'case_parties', 'sessions', 'session_outcomes',
  'tasks_v2', 'task_audit_log', 'task_notifications',
  'activity_logs', 'firm_data', 'settings',
  'documents_v2', 'contracts', 'enforcement_requests',
  'finances', 'invoices', 'vouchers', 'receivables',
  'accounts', 'credit_notes', 'permissions', 'role_permissions',
  'user_permissions', 'file_assets',
  // Missing from schema but referenced in code:
  'agencies', 'invoice_items',
];

for (const t of requiredTables) {
  check(
    tablesInSchema.includes(t.toLowerCase()),
    `جدول: ${t}`,
    '(غير موجود في schema!)'
  );
}
...
check(
  schema.includes('company_id UUID'),
  'company_id موجود في schema (multi-tenancy)'
);
check(
...
const judgmentService = fs.readFileSync('cloud-server/src/services/judgmentAnalyzer.service.ts', 'utf8');
check(
  judgmentService.includes('outcomeType') && !judgmentService.includes('outcome_type'),
  'TypeScript: camelCase (outcomeType)'
);
check(
  judgmentService.includes('hasAppealGrounds'),
  'TypeScript: camelCase (hasAppealGrounds)'
);
check(
  judgmentService.includes('needsExecution'),
  'TypeScript: camelCase (needsExecution)'
);
...
check(
  sessionOutcomesRoute.includes("/apply'"),
  'REST: POST /api/session-outcomes/apply'
);
check(
  sessionOutcomesRoute.includes("/preview'"),
  'REST: POST /api/session-outcomes/preview'
);

const tasksRoute = fs.readFileSync('cloud-server/src/routes/tasks.ts', 'utf8');
check(
  tasksRoute.includes("/by-case/:caseId'"),
  'REST: GET /api/tasks/by-case/:caseId'
);
check(
  tasksRoute.includes("/:id/transition'"),
  'REST: POST /api/tasks/:id/transition'
);
check(
  tasksRoute.includes("/:id/close'"),
  'REST: POST /api/tasks/:id/close'
);
check(
...
check(
  ...
  'REST: GET /api/tasks/:taskId/audit'
);
check(
  tasksRoute.includes("/:taskId/audit/count'"),
  'REST: GET /api/tasks/:taskId/audit/count'
);
...
check(
  apiAdapter.includes('/session-outcomes/apply') &&
  sessionOutcomesRoute.includes('/apply'),
  'session-outcomes/apply endpoint متطابق'
);
check(
  apiAdapter.includes('/session-outcomes/preview') &&
  sessionOutcomesRoute.includes('/preview'),
  'session-outcomes/preview endpoint متطابق'
);
check(
  apiAdapter.includes('/tasks/pending') &&
  tasksRoute.includes('/pending'),
  'tasks/pending endpoint متطابق'
);
check(
  apiAdapter.includes('/tasks/by-case/') &&
  tasksRoute.includes('/by-case/:caseId'),
  'tasks/by-case endpoint متطابق'
);
check(
  (apiAdapter.includes('/tasks/') && apiAdapter.includes('/transition')) &&
  tasksRoute.includes('/:id/transition'),
  'tasks/transition endpoint متطابق'
);
check(
  (apiAdapter.includes('/tasks/') && apiAdapter.includes('/close')) &&
  tasksRoute.includes('/:id/close'),
  'tasks/close endpoint متطابق'
);
check(
  (apiAdapter.includes('/tasks/') && apiAdapter.includes('/cancel')) &&
  tasksRoute.includes('/:id/cancel'),
  'tasks/cancel endpoint متطابق'
);
check(
  (apiAdapter.includes('/tasks/') && apiAdapter.includes('/audit')) &&
  tasksRoute.includes('/:taskId/audit'),
...
check(
  fs.existsSync('cloud-server/src/services/notification.ts'),
  'notification.ts'
);
...
check(
  authMw.includes('export function generateToken'),
  'generateToken مصدرة'
);
check(
  authMw.includes('export async function authMiddleware'),
  'authMiddleware مصدرة'
);

// Check all routes use auth
check(
  tasksRoute.includes('authMiddleware'),
  'tasks route: authMiddleware مستخدم'
);
check(
...
check(
  tasksRoute.includes('companyId') && (tasksRoute.includes('req.auth') || tasksRoute.includes('req.auth')),
...
check(
  sessionOutcomesRoute.includes('activity_logs'),
  'session-outcomes: تسجيل في activity_logs'
);
check(
  tasksRoute.includes('task_audit_log'),
  'tasks: تسجيل في task_audit_log'
);
check(
  tablesInSchema.includes('activity_logs'),
  'activity_logs جدول موجود في schema'
);
check(
  tablesInSchema.includes('task_audit_log'),
  'task_audit_log جدول موجود في schema'
);
...
check(
  indexTs.includes('import { sessionOutcomesRouter }'),
  'sessionOutcomesRouter مستورد'
);
check(
  indexTs.includes('import { tasksRouter }'),
  'tasksRouter مستورد'
);
check(
  indexTs.includes('/api/session-outcomes'),
  '/api/session-outcomes مسجل'
);
check(
  indexTs.includes('/api/tasks'),
  '/api/tasks مسجل'
);
...
check(
  authRoute.includes('logActivity'),
  'logActivity موجودة ومسجلة للأحداث'
);
check(
  authRoute.includes('activity_logs'),
  'activity_logs مستخدمة في auth.ts'
);
check(
...
check(
  loginView.includes('mock_active'),
  'Login.vue: mock_active يستخدم'
);

const mainTs = fs.readFileSync('src/renderer/src/main.ts', 'utf8');
check(
  mainTs.includes('mock_active'),
  'main.ts: mock_active check موجود'
);
...
check(
  judgmentService.includes('CASE_TYPE_DEADLINES'),
  'CASE_TYPE_DEADLINES خريطة المدد'
);
check(
...
check(
  judgmentService.includes('APPEAL_TYPE_MAP'),
  'APPEAL_TYPE_MAP خريطة أنواع الاعتراض'
);
check(
  judgmentService.includes('ابتدائي') && judgmentService.includes('استئنافي') && judgmentService.includes('قطعي') && judgmentService.includes('نهائي'),
