...
export async function createContract(companyId: string, userId: string, data: any) {
  ...
  try {
    ...
    await client.query(contractSql, [
      contractId, companyId, data.contract_type, 'draft', data.title,
      data.case_id || null, data.client_id || null, data.employee_user_id || null,
      data.contract_date || null, data.start_date || null, data.end_date || null,
      data.is_fixed_term || false, data.term_years || null,
      data.total_amount || 0, data.salary_amount || 0, data.salary_due_day || null,
      data.text_content || '', userId, now
    ])

    if (data.contract_type === 'fee_agreement' || data.contract_type === 'employment') {
      const ensureParty = async (partyTypeKey: string, displayName: string, refFields: Record<string, string>) => {
        ...
        await client.query(
          `INSERT INTO contract_parties (id, company_id, party_type_key, ${Object.keys(refFields).join(',')}, display_name, created_at, updated_at)
           VALUES ($1,$2,$3,${Object.values(refFields).map((_, i) => `$${i + 4}`).join(',')},$4,$5,$5)`,
        ...
        await client.query(
          ...
          [participantId, companyId, contractId, partyId,
           partyTypeKey === 'office' ? 'law_firm' : partyTypeKey === 'client' ? 'client' : 'employee',
           partyTypeKey === 'client' ? 'first' : 'second', now]
        )
        return partyId
      }
      ...
      if (data.client_id) {
        const clientRes = await client.query('SELECT name FROM clients WHERE id = $1', [data.client_id])
        if (clientRes.rows[0]) {
