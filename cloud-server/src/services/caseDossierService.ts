...
export async function buildCanonicalCaseDossier(
  companyId: string,
  caseId: string,
  hasFinancialAccess: boolean = true,
  _from?: string,
  _to?: string
): Promise<MasterCaseExecutiveDossier> {
  const caseRes = await query('SELECT * FROM cases WHERE id = $1 AND company_id = $2', [
    caseId,
    companyId
  ])
  ...
  const clientData = caseRow.client_id
    ? await query('SELECT * FROM clients WHERE id = $1 AND company_id = $2', [
        caseRow.client_id,
        companyId
      ]).catch(() => ({ rows: [] }))
    : { rows: [] }
  ...
  const partiesRes = await query(
    'SELECT * FROM case_parties WHERE case_id = $1 AND company_id = $2 ORDER BY created_at ASC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))
  ...
  if (parties.length === 0 && clientData.rows.length > 0) {
    parties = [
      {
        name: clientData.rows[0].name || caseRow.client_name || 'غير مسجل',
        role: caseRow.client_role || 'موكل',
        idNumber: clientData.rows[0].id_number || 'غير مسجل',
        nationality: clientData.rows[0].nationality || 'غير مسجل',
        partyType: 'client',
  ...
  const sessionsRes = await query(
    'SELECT * FROM sessions WHERE case_id = $1 AND company_id = $2 ORDER BY date DESC, time DESC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Judgments
  const judgmentsRes = await query(
    'SELECT * FROM judgments WHERE case_id = $1 AND company_id = $2 ORDER BY judgment_date DESC, created_at DESC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Enforcement requests + financial details
  const enforcementRes = await query(
    `SELECT er.*, efd.amount_instrument, efd.amount_collected_for_client, efd.currency AS enf_currency
     ...
     WHERE er.case_id = $1 AND er.company_id = $2
     ORDER BY er.created_at DESC`,
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Memoranda
  const memorandaRes = await query(
    'SELECT * FROM memoranda WHERE case_id = $1 AND company_id = $2 ORDER BY memo_date DESC, created_at DESC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Experts
  const expertsRes = await query(
    'SELECT * FROM experts WHERE case_id = $1 AND company_id = $2 ORDER BY created_at DESC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Evidence
  const evidenceRes = await query(
    'SELECT * FROM evidence WHERE case_id = $1 AND company_id = $2 ORDER BY created_at DESC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Documents
  const documentsRes = await query(
    'SELECT * FROM documents_v2 WHERE case_id = $1 AND company_id = $2 ORDER BY created_at DESC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Tasks
  const tasksRes = await query(
    'SELECT * FROM tasks_v2 WHERE case_id = $1 AND company_id = $2 ORDER BY due_date ASC NULLS LAST, created_at DESC',
    [caseId, companyId]
  ).catch(() => ({ rows: [] }))

  // Finances (Strictly for office fees with client)
  ...
  // Technical Activity Log (Admin audit ONLY - never used for judicial last action)
  ...
  for (const m of memorandaRes.rows) {
    const mDate = m.memo_date || (m.created_at ? new Date(m.created_at).toISOString().split('T')[0] : '')
    if (mDate) {
      timeline.push({
        date: mDate,
        type: 'مذكرة قضائية',
  ...
  for (const j of judgmentsRes.rows) {
    const jDate = j.judgment_date || (j.created_at ? new Date(j.created_at).toISOString().split('T')[0] : '')
    if (jDate) {
      timeline.push({
        date: jDate,
        type: 'حكم قضائي',
  ...
  for (const er of enforcementRes.rows) {
    const erDate = er.instrument_date || (er.created_at ? new Date(er.created_at).toISOString().split('T')[0] : '')
    if (erDate) {
      timeline.push({
        date: erDate,
        type: 'طلب تنفيذ',
  ...
  // Past sessions
  const pastSessions = sessionsRes.rows
    .filter((s: any) => s.date && s.date < todayStr)
  ...
  // 5. Judgments list
  const judgmentsList = judgmentsRes.rows.map((j: any) => {
  ...
  const enforcementList = enforcementRes.rows.map((er: any) => ({
    id: er.id,
  ...
  // 7. Memoranda list
  const memorandaList = memorandaRes.rows.map((m: any) => ({
    id: m.id,
  ...
  // 8. Experts list
  const expertsList = expertsRes.rows.map((ex: any) => ({
    id: ex.id,
  ...
  const evidenceList = evidenceRes.rows.map((e: any) => ({
    id: e.id,
    title: e.title || 'دليل',
  ...
  const documentsList = documentsRes.rows.map((d: any) => ({
    id: d.id,
    name: d.name || 'مستند',
  ...
  // 10. Tasks list
  const tasksList = tasksRes.rows.map((t: any) => {
  ...
  const lastTechAct =
    actRes.rows.length > 0
      ? {
          actor: actRes.rows[0].actor || 'مستخدم النظام',
          timestamp: actRes.rows[0].timestamp || '',
          action: actRes.rows[0].details || actRes.rows[0].action || 'تحديث تقني'
        }
      : null
  ...
  const sessionsSummaryData = {
    ...
    sessionsList: sessionsRes.rows.map((s: any) => ({
      id: s.id,
      date: s.date || '',
      time: s.time || '',
...
export function renderMasterCaseDossierHtml(
...
<body>
  ...
  <div class="dossier-header">
    <div class="dossier-brand">
      ...
      <div style="font-size: 8.5pt; color: #b45309; font-weight: 700; margin-top: 2px;">تقرير قضية شامل موحد ومعتمد</div>
    </div>
  ...
  <div class="sign-row">
    <div class="sign-block">
      ...
      <div style="margin-top: 24px;">الاسم: ${escapeHtml(d.executiveHeader.responsibleLawyer)}</div>
