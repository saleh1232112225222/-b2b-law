...
export function analyzeJudgment(input: AnalyzeJudgmentInput): JudgmentAnalysis {
  ...
  const degree: JudgmentAnalysis['degree'] = input.judgmentType && (input.judgmentType.includes('قطعي'))
    ? 'قطعي'
  ...
  const favors: 'موكل' | 'خصم' | undefined = input.isForClient === undefined
    ? undefined
  ...
  if (isInitial) {
    if (favors === 'موكل') {
      ...
      if (needsExecution) {
        tasks.push({ title: 'تنفيذ الحكم', description: `بدء إجراءات تنفيذ الحكم. تاريخ البدء: ${todayStr()}.`, priority: 'عاجلة', dueDate: todayStr(), scheduledFor: todayStr() })
        tasks.push({ title: 'تحديد آلية التنفيذ', description: 'تحديد آلية التنفيذ المناسبة (حجز، إخلاء، تحصيل، إلخ).', priority: 'مهمة', dueDate: daysFromNow(3), scheduledFor: daysFromNow(1) })
        tasks.push({ title: 'متابعة إجراءات التنفيذ', description: 'متابعة سير إجراءات التنفيذ بعد 7 أيام.', priority: 'عادية', dueDate: daysFromNow(7), scheduledFor: daysFromNow(7) })
      } else {
        tasks.push({ title: 'أرشفة القضية', description: 'أرشفة القضية بعد الانتهاء منها.', priority: 'عادية', dueDate: daysFromNow(3), scheduledFor: daysFromNow(3) })
      }
      tasks.push({ title: 'تبليغ العميل بالنتيجة', description: 'إبلاغ العميل بالحكم الصادر لصالحه.', priority: 'مهمة', dueDate: todayStr(), scheduledFor: todayStr() })
    } else if (favors === 'خصم') {
      ...
      if (hasAppealGrounds) {
        ...
        tasks.push({ title: `تقديم ${appealType!.label}`, description: `مطلوب تقديم ${appealType!.label} قبل ${deadlines.appealEndDate}.`, priority: 'عاجلة', dueDate: addDaysToDate(serviceDate, appealDays - 5), scheduledFor: todayStr() })
        tasks.push({ title: 'دراسة أسباب الاعتراض', description: 'تحليل الأسباب القانونية للاعتراض.', priority: 'مهمة', dueDate: daysFromNow(appealDays > 30 ? 10 : 5), scheduledFor: todayStr() })
      }
      tasks.push({ title: 'تبليغ العميل بالنتيجة والمخاطر', description: `إبلاغ العميل بالحكم ضد مصلحته${hasAppealGrounds ? ' وخيارات الاعتراض' : ''}.`, priority: 'عاجلة', dueDate: todayStr(), scheduledFor: todayStr() })
    }
  }
  ...
  if (isFinal) {
    if (favors === 'موكل') {
      needsExecution = input.needsExecution === undefined ? false : input.needsExecution
      if (needsExecution) {
        tasks.push({ title: 'تنفيذ الحكم', description: `بدء إجراءات تنفيذ الحكم النهائي.`, priority: 'عاجلة', dueDate: todayStr(), scheduledFor: todayStr() })
        tasks.push({ title: 'تحديد آلية التنفيذ', description: 'تحديد آلية التنفيذ المناسبة.', priority: 'مهمة', dueDate: daysFromNow(3), scheduledFor: daysFromNow(1) })
        tasks.push({ title: 'متابعة إجراءات التنفيذ', description: 'متابعة سير إجراءات التنفيذ.', priority: 'عادية', dueDate: daysFromNow(7), scheduledFor: daysFromNow(7) })
      }
      tasks.push({ title: 'تبليغ العميل بالنتيجة', description: 'إبلاغ العميل بالحكم النهائي.', priority: 'مهمة', dueDate: todayStr(), scheduledFor: todayStr() })
    } else if (favors === 'خصم') {
      // لا يقبل الطعن — مهمة أرشفة
      tasks.push({ title: 'أرشفة القضية', description: 'الحكم نهائي لصالح الخصم ولا يقبل الطعن — أرشفة القضية.', priority: 'عادية', dueDate: daysFromNow(3), scheduledFor: daysFromNow(3) })
      tasks.push({ title: 'تبليغ العميل بالنتيجة والمخاطر', description: 'إبلاغ العميل بالحكم النهائي ضد مصلحته.', priority: 'عاجلة', dueDate: todayStr(), scheduledFor: todayStr() })
    }
  }
  ...
  if (isInitial) {
    ...
    if (favors === 'خصم') summary += hasAppealGrounds ? ' | يوجد سبب اعتراض' : ' | لا يوجد سبب اعتراض'
  }
  if (isFinal && favors === 'موكل') summary += needsExecution ? ' | يحتاج تنفيذ' : ' | لا يحتاج تنفيذ'
  ...
  return {
    outcomeType, degree, favors, needsExecution, hasAppealGrounds,
    appealType: appealType?.type,
    deadlines: deadlines.appealDeadlineDays ? deadlines as JudgmentAnalysis['deadlines'] : undefined,
    tasks,
    summary,
  }
}
...
function analyzeNonJudgmentOutcome(outcomeType: JudgmentAnalysis['outcomeType']): JudgmentAnalysis {
  const tasks: GeneratedTask[] = []
  if (outcomeType === 'قرار') {
    tasks.push({ title: 'تحليل القرار الصادر', description: 'مراجعة القرار الصادر وتحليل آثاره.', priority: 'مهمة', dueDate: todayStr(), scheduledFor: todayStr() })
    tasks.push({ title: 'تبليغ العميل بالقرار', description: 'إبلاغ العميل بالقرار.', priority: 'مهمة', dueDate: todayStr(), scheduledFor: todayStr() })
  } else if (outcomeType === 'حجز للحكم') {
    tasks.push({ title: 'متابعة تاريخ النطق بالحكم', description: 'متابعة الجلسة المحددة للنطق بالحكم.', priority: 'عاجلة', scheduledFor: todayStr() })
  } else if (outcomeType === 'تأجيل') {
    tasks.push({ title: 'متابعة تاريخ الجلسة الجديدة', description: 'متابعة الجلسة الجديدة المحددة.', priority: 'مهمة', scheduledFor: todayStr() })
  } else if (outcomeType === 'تبليغ / إجراء إداري') {
    tasks.push({ title: 'متابعة إجراءات التبليغ', description: 'متابعة إجراءات التبليغ.', priority: 'مهمة', scheduledFor: todayStr() })
  } else {
    tasks.push({ title: 'مراجعة النتيجة وتحليلها', description: 'مراجعة النتيجة غير المصنفة.', priority: 'مهمة', dueDate: daysFromNow(1), scheduledFor: todayStr() })
    tasks.push({ title: 'تبليغ العميل بنتيجة الجلسة', description: 'إبلاغ العميل بنتيجة الجلسة.', priority: 'عادية', dueDate: todayStr(), scheduledFor: todayStr() })
  }
  return {
    outcomeType, tasks,
...
export async function saveGeneratedTasks(
  ...
  for (const t of tasks) {
    ...
    await query(
      ...
      [id, companyId, caseId || null, t.title, t.description,
        t.priority === 'عاجلة' ? 'high' : t.priority === 'مهمة' ? 'medium' : 'low',
        t.dueDate || null, t.scheduledFor ? `${t.scheduledFor}T00:00:00Z` : null, createdBy]
    )
    ids.push(id)
  }
  return ids
