...
router.post('/expenses',
  ...
  async (req: any, res) => {
    try {
      ...
      `, [companyId, category, description, amount, expense_date || new Date().toISOString().split('T')[0], paid_by, receipt_number, notes, userId])
...
router.post('/partners',
  requirePermission('manage_office'),
  async (req: any, res) => {
    try {
      const { companyId } = req.auth
    ...
    try {
      ...
      res.json({
        ...
        summary: {
          ...
          collection_rate: Number(totalServicesDue.rows[0]?.total || 0) > 0
            ? Math.round((Number(totalServicesPaid.rows[0]?.total || 0) / Number(totalServicesDue.rows[0]?.total || 0)) * 100)
            : 0
        },
...
router.post('/distributions',
  ...
  async (req: any, res) => {
    try {
      ...
      res.json({ success: true, total_revenue: totalRevenue, total_expenses: totalExpenses, net_profit: netProfit })
    } catch (err: any) {
