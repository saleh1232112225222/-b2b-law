...
async function upsertClientAccount(companyId: string, clientId: string) {
  ...
  const status = balanceNum <= 0 ? 'settled'
    : overdue_amount > 0 ? 'overdue' : 'active'
  ...
  async (req: any, res) => {
    try {
      ...
      const voucherResult = await query(`
        ...
        RETURNING id
      `, [
        companyId, eng.client_id, eng.case_id, voucherNumber,
        paidAmount, payment_method || 'cash',
      ...
      `, [companyId, id, payment_schedule_id || null, paidAmount,
...
router.post('/engagements/:id/installments',
  ...
  async (req: any, res) => {
    try {
      ...
      // حذف الأقساط القديمة
      await query('DELETE FROM payment_schedules WHERE legal_engagement_id = $1 AND company_id = $2', [id, companyId])
  ...
  async (req: any, res) => {
    try {
      ...
      await query(`
        ...
        WHERE id = $7
      `, [
        newComp, newRemaining, originalCompensation,
