...
function verifyOAuthState(req: Request, res: Response): boolean {
  ...
  // Parse nonce from HttpOnly cookie
  ...
  if (!cookieNonce) {
    console.warn('[OAUTH_STATE] Verification failed: __oauth_nonce cookie missing from request headers')
    return false
  }
  ...
  // 1. Verify browser binding: cookie nonce MUST match state nonce strictly
  if (stateNonce !== cookieNonce) {
    console.warn('[OAUTH_STATE] Verification failed: state nonce does not match cookie nonce (cookieMatched=false)')
    return false
  }
  ...
  // 3. Verify HMAC integrity: state was generated by this server with OAUTH_STATE_SECRET
