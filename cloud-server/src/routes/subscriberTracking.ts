 ...
 */
subscriberTrackingRouter.get('/:userId/overview', requireAdminRole, async (req: Request, res: Response) => {
  try {
 ...
 */
subscriberTrackingRouter.get('/:userId/login-logs', requireAdminRole, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params
 ...
 */
subscriberTrackingRouter.get('/:userId/activity-logs', requireAdminRole, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params
    const limit = Math.min(parseInt(req.query.limit as string) || 50, 200)
    const offset = parseInt(req.query.offset as string) || 0
