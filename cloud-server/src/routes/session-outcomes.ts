...
sessionOutcomesRouter.post('/apply', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { sessionId, result, notes, judgmentData, dismissalDecision, serviceData, caseType } = req.body
    ...
    const sessionRes = await client.query('SELECT case_id, company_id FROM sessions WHERE id = $1', [sessionId])
    ...
    if (analysis.tasks.length > 0) {
      for (const t of analysis.tasks) {
        ...
        await client.query(
          ...
          [taskId, companyId, caseId || null, t.title, t.description,
            t.priority === 'عاجلة' ? 'high' : t.priority === 'مهمة' ? 'medium' : 'low',
            t.dueDate || null, t.scheduledFor ? `${t.scheduledFor}T00:00:00Z` : null, userId]
        )
      }
    }
    ...
    await client.query(
      ...
      [uuidv4(), companyId, 'SESSION_OUTCOME_APPLIED', 'sessions', `تسجيل نتيجة الجلسة: ${result}`, req.auth!.username, userId, JSON.stringify({ outcomeId, sessionId, caseId, result, analysis: analysis.summary })]
    )
