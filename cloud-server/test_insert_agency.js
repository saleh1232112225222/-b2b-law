...
async function run() {
  try {
    ...
    const clientsRes = await client.query('SELECT id FROM clients WHERE company_id = $1 LIMIT 1', [companyId]);
    ...
    const usersRes = await client.query('SELECT id FROM users WHERE username = $1 AND company_id = $2', ['slaehmap', companyId]);
