...
async function login(page: Page) {
  ...
  // Fill login form
  const usernameField = page.locator('input[type="text"], input[name="username"], input:not([type="password"])').first()
  await usernameField.fill('OpenCode')
  
  const passField = page.locator('input[type="password"]').first()
  await passField.fill('OpenCode@111')
  
  const loginBtn = page.locator('button[type="submit"], button:has-text("دخول"), button:has-text("تسجيل"), .v-btn--is-elevated').first()
...
test.describe('المرحلة 1: الأساسيات والمسارات الحرجة', () => {
  ...
  test('1.1 تسجيل الدخول على شاشة Pixel 8', async ({ browser }) => {
    ...
    try {
      ...
      // Check current page
      const pageTitle = await page.locator('.v-toolbar-title, .mobile-header-title, h1, h2').first().textContent().catch(() => 'unknown')
  ...
  test('1.2 اختبار على 5 أحجام شاشة مختلفة', async ({ browser }) => {
    ...
    for (const size of sizes) {
      ...
      const bodyText = await pg.locator('body').textContent().catch(() => '')
  ...
  test('1.4 اختبار القائمة الجانبية (Drawer)', async ({ browser }) => {
    ...
    const items = ['المهام', 'المالية', 'المستندات', 'المذكرات', 'العقود', 'التنفيذ', 'الملفات', 'التقارير', 'الملف الشخصي', 'الإعدادات']
    
    for (const item of items) {
      const drawerBtn = pg.locator(`.v-navigation-drawer .v-list-item:has-text("${item}"), .v-list-item:has-text("${item}")`)
  ...
  test('1.5 اختبار تسجيل الخروج', async ({ browser }) => {
    ...
    // Click logout
    const logoutBtn = pg.locator('button:has-text("تسجيل الخروج"), .v-btn:has-text("الخروج")').first()
