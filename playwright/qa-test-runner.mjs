...
async function phase01_login(browser) {
  ...
  try {
    ...
    let usernameField = page
      .locator(
        'input[autocomplete="username"], input[name="username"], input:not([type="password"])'
      )
      .first()
    ...
    const loginBtn = page
      .locator(
        'button[type="submit"], button:has-text("دخول"), button:has-text("تسجيل الدخول"), .v-btn--disabled + .v-btn, .v-btn.v-btn--elevated'
      )
      .first()
    ...
    if (btnCount > 0) {
      ...
      for (const size of sizes) {
        ...
        const sHasBottomNav = await sPage
          .locator('.mobile-bottom-nav, .v-bottom-navigation')
          .count()
      ...
      for (const nav of navButtons) {
        const btn = navPage
          .locator(
            `.v-bottom-navigation button:has-text("${nav.label}"), .mobile-bottom-nav button:has-text("${nav.label}")`
          )
          .first()
      ...
      if ((await hamburger.count()) > 0) {
        ...
        const drawerItems = await drawPage
          .locator('.v-navigation-drawer .v-list-item, .mobile-drawer .v-list-item')
          .count()
      ...
      const logoutBtn = drawPage
        .locator('button:has-text("تسجيل الخروج"), .v-list-item:has-text("الخروج")')
        .first()
