...
async function dismissOverlays(page) {
  ...
  try {
    ...
    const continueBtn = page.locator('button:has-text("متابعة التصفح"), button:has-text("إغلاق"), .v-btn:has-text("العودة")').first()
...
async function clickNav(page, text) {
  await dismissOverlays(page)
  try {
    const btn = page.locator(`button:has-text("${text}"), a:has-text("${text}"), .v-btn:has-text("${text}")`).first()
...
async function main() {
  ...
  try {
    ...
    if (!loggedIn) {
      addIssue('فشل تسجيل الدخول', 'لم يتم تجاوز صفحة الدخول', 'الدخول إلى /dashboard', 'حرجة', 'قبل الإطلاق')
      return
    }
    ...
    const dashText = await page.locator('.v-main').textContent().catch(() => '')
    ...
    if (await clickNav(page, 'العملاء')) {
      ...
      console.log('  ✓ العملاء')
      const cText = await page.locator('.v-main').textContent().catch(() => '')
    ...
    try {
      smallButtons = await page.evaluate(() => {
        ...
        let s = 0
        btns.forEach(b => { const r = b.getBoundingClientRect(); if (r.width < 40 && r.height < 40) s++ })
        return s
      })
    } catch(e) {}
    ...
    if (await searchField.count() > 0) {
      ...
      if (val.includes('<script>')) addIssue('XSS غير محمي في البحث', `يقبل: ${val}`, 'ترميز HTML', 'مرتفعة', 'مرتفعة')
    ...
    await wait(1500); await dismissOverlays(page)
  ...
  console.log(`المشاكل: ${issues.length} (حرجة:${c.length} مرتفعة:${h.length} متوسطة:${m.length} منخفضة:${l.length})`)
