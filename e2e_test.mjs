...
const step = async (num, desc) => {
  console.log(`\n=== ${num}. ${desc} ===`)
  await p.screenshot({
    path: `e2e-${String(num).padStart(2, '0')}-${desc.slice(0, 30)}.png`,
    fullPage: true
  })
}
...
await p
  .goto('https://b2b-law.netlify.app/#/register', { waitUntil: 'networkidle', timeout: 15000 })
  .catch(() => {})
...
const registerForm = await p
  .locator('input[placeholder*="اسم مكتب"]')
...
await p
  .goto('https://b2b-law.netlify.app/#/login', { waitUntil: 'networkidle', timeout: 15000 })
  .catch(() => {})
await p.waitForTimeout(2000)
...
await p
  .goto('https://b2b-law.netlify.app/#/clients', { waitUntil: 'networkidle', timeout: 15000 })
  .catch(() => {})
await p.waitForTimeout(3000)

const addClient = p
  .locator(
    'button:has-text("إضافة"), button:has-text("موكل جديد"), button:has-text("إضافة موكل"), .v-btn .v-icon--start'
  )
  .first()
...
await p
  .goto('https://b2b-law.netlify.app/#/cases', { waitUntil: 'networkidle', timeout: 15000 })
  .catch(() => {})
await p.waitForTimeout(3000)

const addCase = p
  .locator('button:has-text("إضافة"), button:has-text("قضية جديدة"), button:has-text("إضافة قضية")')
  .first()
...
log('\nمحاولة إنشاء جلسة...')
await p
  .goto('https://b2b-law.netlify.app/#/sessions', { waitUntil: 'networkidle', timeout: 15000 })
  .catch(() => {})
await p.waitForTimeout(3000)

const addSession = p
  .locator('button:has-text("إضافة"), button:has-text("جلسة جديدة"), button:has-text("إضافة جلسة")')
  .first()
