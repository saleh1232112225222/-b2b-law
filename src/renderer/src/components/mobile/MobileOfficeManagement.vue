  ...
  <div class="pa-2">
    ...
    <template v-else-if="dashboard">
      ...
      <v-row dense class="mb-3">
        <v-col cols="6">
          <v-select v-model="selectedMonth" :items="months" item-title="title" item-value="value"
            label="الشهر" variant="outlined" density="compact" hide-details @update:model-value="loadData" />
        </v-col>
        <v-col cols="6">
          <v-select v-model="selectedYear" :items="years" label="السنة" variant="outlined" density="compact"
            hide-details @update:model-value="loadData" />
        </v-col>
      </v-row>
      ...
      <v-row dense class="mb-3">
        ...
        <v-col cols="4">
          <v-card variant="outlined" class="pa-3 text-center rounded-lg">
            ...
            <div class="text-subtitle-2 font-weight-black" :class="dashboard.summary.net_profit >= 0 ? 'text-success' : 'text-error'">
      ...
      <div class="mb-3">
        ...
        <v-card v-if="dashboard.partners.length === 0" variant="outlined" class="pa-4 text-center rounded-lg">
          <div class="text-caption text-medium-emphasis">لا يوجد شركاء مسجلين</div>
        </v-card>
        <v-card v-else v-for="p in dashboard.partners" :key="p.partner_id" variant="outlined" class="mb-2 pa-3 rounded-lg">
      ...
      <div class="mb-3">
        ...
        <v-card v-else v-for="e in expenses.slice(0, 5)" :key="e.id" variant="outlined" class="mb-2 pa-3 rounded-lg">
          <div class="d-flex justify-space-between align-center">
            <div>
            ...
            <div class="text-end">
              <div class="text-caption text-error font-weight-bold">{{ formatMoney(e.amount) }}</div>
        ...
        <v-card class="rounded-t-xl">
          ...
          <v-card-text>
            <v-select v-model="newExpense.category" :items="expenseCategories" item-title="title" item-value="value"
              label="التصنيف" variant="outlined" density="compact" class="mb-3" />
            <v-text-field v-model="newExpense.description" label="الوصف" variant="outlined" density="compact" class="mb-3" />
            <v-text-field v-model.number="newExpense.amount" label="المبلغ" type="number" variant="outlined" density="compact" class="mb-3" />
            <v-text-field v-model="newExpense.expense_date" label="التاريخ" type="date" variant="outlined" density="compact" class="mb-3" />
            <v-text-field v-model="newExpense.paid_by" label="الدفع بواسطة" variant="outlined" density="compact" class="mb-3" />
            <v-textarea v-model="newExpense.notes" label="ملاحظات" variant="outlined" density="compact" rows="2" />
          </v-card-text>
...
const getCategoryLabel = (key: string) => expenseCategories.find(c => c.value === key)?.title || key
