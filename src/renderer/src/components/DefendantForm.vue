...
const oneOfIdOrPhoneRule = computed(() => {
  return () => {
    const idn = String((props.modelValue as any).id_number || '').trim()
    const phone = String((props.modelValue as any).phone || '').trim()
    return idn || phone ? true : 'رقم الجوال أو رقم الهوية مطلوب'
  }
})
