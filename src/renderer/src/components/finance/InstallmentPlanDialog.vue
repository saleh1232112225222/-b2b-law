      ...
      <v-card-text>
        <v-card variant="outlined" class="pa-4 mb-4 rounded-xl bg-grey-lighten-5">
          ...
          <div class="text-h5 font-weight-black text-primary">
            ...
            <span class="text-body-2">ريال</span>
          </div>
        </v-card>
        ...
        <v-alert
          v-if="
            installments.length > 0 && totalInstallments !== (engagement?.remaining_amount || 0)
          "
          type="warning"
          density="compact"
          class="mt-3"
...
const handleSave = async () => {
  saving.value = true
  try {
    await financeStore.createInstallments(props.engagement.id, installments.value, frequency.value)
    emit('save')
