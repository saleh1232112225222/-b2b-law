  ...
  <div>
            ...
            <div class="d-flex align-center mb-3">
              ...
              <div>
                ...
                <div class="text-h5 font-weight-black text-error">
                  {{ formatCurrency(dashboard.summary.total_expenses) }}
                </div>
              </div>
            </div>
            ...
            <div class="d-flex align-center mb-3">
              ...
              <div>
                ...
                <div class="text-h5 font-weight-black text-accent">
                  {{ formatCurrency(dashboard.summary.yearly_revenue) }}
                </div>
              </div>
            </div>
      ...
      <v-card class="glass-card overflow-hidden">
        ...
        >
          <v-tab value="budget" class="font-weight-black">
            <v-icon size="18" class="me-2">mdi-chart-pie</v-icon> ميزانية التوزيع وأصحاب الربح
          </v-tab>
          ...
          <v-window-item value="budget">
            ...
            <div class="mb-8">
              <div class="d-flex justify-space-between align-center mb-4">
                <h3 class="text-h6 font-weight-black text-gold">
                  انتاجية ونسبة مساهمة الشركاء والمحامين
                </h3>
                ...
                <tbody>
                  <tr v-for="p in dashboard.partners" :key="p.partner_id">
                    ...
                    <td class="text-right font-weight-black text-error">
                      {{ formatCurrency(Math.abs(p.distributable_amount)) }}
                    </td>
                    <td class="text-right">
                      <v-progress-linear
                        :model-value="p.share_percentage"
                        color="accent"
                        height="8"
                        rounded
            ...
            <div>
              <div class="d-flex justify-space-between align-center mb-4">
                <h3 class="text-h6 font-weight-black text-gold">
                  استهلاكات الميزانية التشغيلية للمؤسسة
                </h3>
            ...
            <v-row dense>
              <v-col v-for="p in partners" :key="p.id" cols="12" sm="6" md="4">
                <v-card class="glass-card pa-5" elevation="0">
                  <div class="d-flex align-center mb-3">
                    <v-avatar
                      :color="p.role === 'managing_partner' ? 'accent' : 'primary'"
                      size="48"
                      class="me-3"
                    >
                      <span class="text-white font-weight-black text-h6">{{
                        p.name?.charAt(0)
                      }}</span>
                    </v-avatar>
                    <div>
            ...
            <v-table v-else class="bg-transparent">
              ...
              <tbody>
                <tr v-for="d in distributions" :key="d.id">
                  ...
                  <td class="text-right text-accent font-weight-black">
                    {{ d.share_percentage }}%
                  </td>
                  <td class="text-right font-weight-black text-white">
                  ...
                  <td class="text-right">
                    <v-chip
                      size="x-small"
                      :color="d.distributed ? 'success' : 'warning'"
                      class="font-weight-black"
                    >
    ...
    <v-dialog v-model="showAddPartner" max-width="500" persistent>
      <v-card class="rounded-xl">
        ...
        <v-card-text>
          ...
          />
          <v-text-field
            v-model.number="newPartner.share_percentage"
            label="نسبة الربح (%)"
            type="number"
            variant="outlined"
            class="mb-3"
        ...
        <v-card-actions class="pa-4 pt-0">
          ...
          <v-btn
            color="accent"
            :loading="saving"
            :disabled="!newPartner.name || !newPartner.share_percentage"
    ...
    <v-dialog v-model="showAddContribution" max-width="600" persistent>
      <v-card class="rounded-xl">
        ...
        <v-card-text>
          <v-select
            v-model="newContribution.partner_id"
            :items="partners"
            item-title="name"
            item-value="id"
            label="الشريك"
          ...
          />
          <v-select
            v-model="newContribution.contribution_type"
        ...
        <v-card-actions class="pa-4 pt-0">
          ...
          <v-btn
            color="accent"
            :loading="saving"
            :disabled="!newContribution.partner_id || !newContribution.contribution_type"
...
const newContribution = ref<any>({
  partner_id: '',
  contribution_type: 'client_acquired',
  description: '',
  amount: 0
})
...
const expenseCategories = [
  ...
  {
    title: 'مستلزمات قانونية',
    value: 'legal_supplies',
    key: 'legal_supplies',
    label: 'مستلزمات قانونية',
    icon: 'mdi-scale',
    color: 'teal'
  },
  {
...
const getCategoryLabel = (key: string) =>
  expenseCategories.find((c) => c.value === key)?.label || key
const getCategoryColor = (key: string) =>
  expenseCategories.find((c) => c.value === key)?.color || 'grey'
const getRoleLabel = (role: string) => {
  const r = partnerRoles.find((r) => r.value === role)
  return r?.title || role || 'شريك'
}
const getContributionTypeLabel = (type: string) =>
  contributionTypes.find((c) => c.value === type)?.title || type
