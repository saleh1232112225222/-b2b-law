<template>
  <v-container fluid class="pa-6 pb-12 rtl">
    <PrintReportFrame title="كشف المستندات المودعة" />
      ...
      <v-col>
        <div class="d-flex align-center">
          <div class="glass-panel-light pa-4 rounded-xl me-5 border-gold opacity-20">
            <LucideIcon name="folder-search" :size="36" class="text-accent" />
          </div>
          <div>
            <h1 class="text-h5 font-weight-black text-gold mb-1">تقرير وإحصائيات المستندات</h1>
            <p class="text-subtitle-1 text-gold opacity-60 font-weight-black">
              جرد شامل لكافة المرفقات، الأدلة الرقمية، والوثائق المودعة في الأرشفة الذكية
            </p>
          </div>
        </div>
      </v-col>
    ...
    >
        ...
        <v-col cols="12" md="3">
          ...
          >
            <template #prepend-inner>
              <LucideIcon name="link" :size="20" class="text-gold me-2" />
            </template>
          </v-select>
        </v-col>
        <v-col cols="12" md="3">
      ...
      <div class="d-flex align-center mb-4">
        ...
        <span class="text-h6 font-weight-black text-white">بيان المستندات المكتشفة</span>
      </div>
      ...
      >
        <v-table density="comfortable" class="premium-table">
          <thead>
            <tr>
              <th class="text-right text-gold font-weight-black">تاريخ الإضافة</th>
              <th class="text-right text-gold font-weight-black">اسم المستند</th>
              <th class="text-right text-gold font-weight-black">نوع الارتباط</th>
              <th class="text-right text-gold font-weight-black">السياق المرتبط</th>
            </tr>
          </thead>
          <tbody>
            ...
            <template v-else>
              <tr v-if="safeLength(rows) === 0">
                <td colspan="4" class="text-center py-12 text-gold opacity-20">
                  لا توجد مستندات مسجلة ضمن هذه المعايير
                </td>
              </tr>
              <tr v-for="r in safeArray(rows)" :key="(r as any).id" class="premium-hover-row">
                ...
                <td class="text-body-2 font-weight-black text-white">
                  <div class="d-flex align-center">
                    <LucideIcon
                      :name="getFileIcon((r as any).name)"
                      :size="18"
                      class="me-3 text-gold"
                    />
                    {{ (r as any).name }}
                  </div>
                </td>
                <td>
                  ...
                  >
                    {{ getLinkTypeLabel((r as any).link_type) }}
                  </v-chip>
                </td>
                <td
                  class="text-caption text-white opacity-60 text-truncate"
                  style="max-width: 340px"
                >
                  {{ (r as any).context_label || (r as any).linked_title || '-' }}
                </td>
              </tr>
            </template>
          </tbody>
      ...
      >
        ...
        <span class="text-subtitle-2 font-weight-black text-gold opacity-60"
          >إجمالي المستندات المكتشفة في التقرير:</span
        >
        <v-spacer />
        <span class="text-h6 font-weight-black text-white">{{ safeLength(rows) }} مستند</span>
      </div>
    </v-card>
...
const error = ref('')
const parentOptions = ref<{ title: string; value: string }[]>([])

const linkTypes = [
  { title: 'مرتبط بقضية', value: 'case' },
  { title: 'مرتبط بمهمة', value: 'task' },
  { title: 'مرتبط بجلسة', value: 'session' },
  { title: 'بدون ارتباط مباشر', value: 'none' }
]

const loadParentOptions = async (): Promise<void> => {
  try {
    if (linkType.value === 'case') {
      const data = await (window as any).api.reports.listCases()
      parentOptions.value = safeArray(data).map((r: any) => ({
        value: r.id,
        title: `${r.case_number || r.id} — ${r.client_name || ''}`
      }))
    } else if (linkType.value === 'session') {
      const data = await (window as any).api.reports.listSessions()
      parentOptions.value = safeArray(data).map((s: any) => ({
        value: s.id,
        title: `جلسة ${s.date || ''} — ${s.status || ''}`
      }))
    } else if (linkType.value === 'task') {
      const data = await (window as any).api.reports.listTasks()
      parentOptions.value = safeArray(data).map((t: any) => ({
        value: t.id,
        title: `${t.title || t.id}`
      }))
    } else {
...
watch(linkType, (newVal) => {
  parentId.value = null
  if (newVal) loadParentOptions()
  else parentOptions.value = []
})
...
const load = async (): Promise<void> => {
  ...
  error.value = ''
  try {
    const res = await (window as any).api.reports.getDocumentsReport({
      linkType: linkType.value || undefined,
    ...
    error.value = (e as Error)?.message || 'فشل تحميل تقرير المستندات'
  } finally {
    loading.value = false
  }
}
...
const exportCsv = async (): Promise<void> => {
  ...
  try {
    const res = await (window as any).api.reports.exportCsv('documents-report.csv', rows.value)
...
const getFileIcon = (name: string | undefined): string => {
  const n = (name || '').toLowerCase()
  if (n.endsWith('.pdf')) return 'file-text'
  if (n.endsWith('.doc') || n.endsWith('.docx')) return 'file-signature'
  if (n.endsWith('.xls') || n.endsWith('.xlsx')) return 'file-spreadsheet'
  if (n.endsWith('.jpg') || n.endsWith('.png') || n.endsWith('.jpeg')) return 'image'
  return 'file'
}

const getLinkTypeLabel = (type: string | undefined): string => {
  if (type === 'case') return 'قضية'
  if (type === 'task') return 'مهمة'
  if (type === 'session') return 'جلسة'
  return 'مستند عام'
}

onMounted(() => {
  if (linkType.value) loadParentOptions()
  load()
})
</script>
