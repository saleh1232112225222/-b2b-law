  ...
  <div>
    <div class="d-flex align-center justify-space-between mb-4">
      <div class="text-h6 font-weight-black text-primary">
        <LucideIcon name="users" :size="24" class="text-primary me-2" /> أطراف القضية (موكلين وخصوم)*
      </div>
      <div class="d-flex ga-4">
        <v-btn size="large" variant="tonal" color="primary" class="font-weight-black rounded-xl px-6" @click="addParty('client')">
          <LucideIcon name="plus" :size="20" class="me-2" /> إضافة موكل
        </v-btn>
        <v-btn size="large" variant="tonal" color="accent" class="font-weight-black rounded-xl px-6" @click="addParty('opponent')">
          <LucideIcon name="plus" :size="20" class="me-2" /> إضافة خصم
        </v-btn>
      </div>
    </div>

    <v-alert v-if="parties.length === 0" type="warning" variant="tonal" class="rounded-xl mb-6 glass-card border shadow-sm">
      يجب إضافة طرف واحد على الأقل (موكل) لحفظ القضية.
    </v-alert>

    <div v-for="(party, idx) in parties" :key="idx" class="party-item mb-6 pa-6 rounded-xl glass-card border position-relative overflow-hidden premium-hover shadow-premium">
      <div class="position-absolute text-primary" style="top: -20px; right: -20px; opacity: 0.06; pointer-events: none">
        <LucideIcon :name="party.party_type === 'client' ? 'user-check' : 'user-x'" :size="120" />
      </div>
      <v-btn variant="tonal" color="error" size="small" class="position-absolute rounded-lg glass-card" style="top: 12px; left: 12px; z-index: 2" @click="removeParty(idx)">
      ...
      <v-row dense>
        <v-col cols="12" md="2">
          <v-select
            ...
            :items="[{ title: 'موكل', value: 'client' }, { title: 'خصم', value: 'opponent' }]"
            label="نوع الطرف"
            variant="outlined"
            density="compact"
            hide-details
          ...
          >
            <template #prepend-inner>
              <LucideIcon :name="party.party_type === 'client' ? 'user-check' : 'user-x'" :size="16" class="text-primary me-2" />
            </template>
          </v-select>
        </v-col>
        ...
        <template v-else>
          <v-col cols="12" md="5">
            <div class="d-flex ga-4 align-center">
              ...
              <v-btn size="small" variant="tonal" color="primary" class="rounded-lg h-40 px-3" @click="$emit('quickAddDefendant', idx)">
