  ...
  >
    ...
    <div v-else class="d-flex flex-column ga-4">
      <v-card
        ...
        :key="item.id"
        elevation="0"
        class="glass-card pa-4 rounded-xl border premium-lift position-relative"
      >
        ...
        <div class="mb-3">
          <div class="d-flex align-center flex-wrap ga-2">
            ...
            <v-btn
              ...
              variant="text"
              color="accent"
              class="px-0 font-weight-bold text-body-2"
            ...
            <span v-else class="font-weight-bold text-body-2 text-white">{{
              getCaseClientName(item) || 'بدون موكل'
            }}</span>
            <span v-if="getCaseOpponentName(item)" class="text-caption text-white opacity-60 mx-1"
              >ضد</span
            >
            <span
              v-if="getCaseOpponentName(item)"
              class="text-body-2 text-text-muted font-weight-bold"
              >{{ getCaseOpponentName(item) }}</span
            >
          </div>
          <div
            v-if="getCaseExtraPartiesCount(item) > 0"
            class="text-caption text-text-muted opacity-70 mt-1"
          >
        ...
        <div class="d-flex flex-wrap ga-x-4 ga-y-2 border-t pt-3 border-white-alpha-10">
          ...
          <div class="d-flex align-center ga-1 text-caption">
            ...
            <span class="text-white font-weight-bold">{{
              item.main_classification || 'غير محدد'
            }}</span>
          </div>
          <div class="d-flex align-center ga-1 text-caption">
          ...
          <div class="d-flex align-center ga-1 text-caption">
            ...
            <span class="text-white font-weight-bold">
              ...
              <span v-if="item.registration_date_hijri" class="text-accent opacity-90 ms-1"
                >({{ item.registration_date_hijri }} هـ)</span
              >
            </span>
          </div>
        </div>
