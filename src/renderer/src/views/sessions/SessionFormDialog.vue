<template>
  <v-dialog :model-value="show" width="90%" max-width="800" persistent scrollable @update:model-value="$emit('update:show', $event)">
    <v-card class="modal-card overflow-hidden">
      <div class="modal-header-solid d-flex align-center py-5 px-8">
        <div class="bg-accent-alpha pa-2 rounded-lg me-4">
          <LucideIcon :name="isEditing ? 'edit-3' : 'calendar-plus'" :size="24" class="text-primary" />
        </div>
        <span class="text-h5 font-weight-black text-pure-black">{{ isEditing ? 'تعديل بيانات الجلسة المجدولة' : 'إدراج موعد جلسة قضائية جديد' }}</span>
        <v-spacer />
          ...
          <v-row>
            <v-col v-if="caseBlockStatus.is_blocked" cols="12">
              <v-alert type="error" variant="flat" class="rounded-lg mb-6 border-dashed">
                ...
                <div class="text-subtitle-1 font-weight-black mb-1">حظر مهني نشط: {{ caseBlockStatus.reason }}</div>
                <div class="text-tiny opacity-90 font-weight-black leading-relaxed">يجب استكمال إغلاق الجلسات السابقة لهذه القضية في لوحة القيادة أولاً لضمان دقة الأرشيف.</div>
                <div class="mt-4">
                  <v-btn color="white" variant="flat" size="small" class="font-weight-black rounded-md text-error premium-lift" @click="$router.push('/briefing')">توجه للوحة القيادة</v-btn>
                </div>
              </v-alert>
            </v-col>

            <v-col cols="12">
              <v-label class="mb-2 font-weight-black text-pure-black">ملف القضية المستهدفة*</v-label>
              <v-autocomplete v-model="editItem.case_id" :items="safeArray(caseOptions)" item-title="display" item-value="id" placeholder="ابحث عن رقم القضية أو اسم العميل..." variant="outlined" class="premium-input-solid" :rules="[(v: any) => !!v || 'القضية مطلوبة لربط الموعد']" :custom-filter="arabicFilter" no-data-text="لا يوجد قضايا حالياً" required>
            ...
            <v-col cols="12" md="6">
              ...
              <v-select v-model="editItem.responsible_user_id" :items="assignableUsers" :item-title="getUserDisplayName" item-value="id" variant="outlined" class="premium-input-solid" clearable :loading="assignableUsersLoading">
                <template #prepend-inner>
            ...
            <v-col cols="12" md="6">
              ...
              <v-text-field v-model="editItem.time" type="time" variant="outlined" class="premium-input-solid">
                <template #prepend-inner>
            ...
            <v-col cols="12" md="6">
              <v-label class="mb-2 font-weight-black text-pure-black">القاعة / الدائرة القضائية</v-label>
              <v-text-field v-model="editItem.court_room" placeholder="مثال: القاعة الخامسة، الدائرة السادسة" variant="outlined" class="premium-input-solid">
                <template #prepend-inner>
            ...
            <v-col cols="12" md="6">
              ...
              <v-select v-model="editItem.type" :items="SESSION_TYPES" variant="outlined" class="premium-input-solid" required>
                <template #prepend-inner>
            ...
            <v-col cols="12" md="6">
              ...
              <v-select v-model="editItem.status" :items="SESSION_STATUSES" variant="outlined" class="premium-input-solid" required>
                <template #prepend-inner>
            ...
            <v-col cols="12">
              <v-label class="mb-2 font-weight-black text-pure-black">رابط الجلسة الرقمية (ناجز / تيمز)</v-label>
              <v-text-field v-model="editItem.meeting_link" placeholder="https://..." variant="outlined" class="premium-input-solid" hint="سيتم تفعيل زر 'الانضمام المباشر' في الأجندة عند إضافة الرابط" persistent-hint>
                <template #prepend-inner>
            ...
            <v-col cols="12">
              <v-label class="mb-2 font-weight-black text-pure-black">ملاحظات ووقائع الجلسة</v-label>
              <v-textarea v-model="editItem.notes" variant="outlined" rows="2" class="premium-input-solid">
                <template #prepend-inner>
            ...
            <v-col v-if="editItem.status === 'تمت' || editItem.status === 'مؤجلة'" cols="12">
              <v-label class="mb-2 font-weight-black text-accent">قرار اللجنة / سبب التأجيل</v-label>
              <v-text-field v-model="editItem.result" variant="outlined" class="premium-input-solid" color="accent">
                <template #prepend-inner>
                  <LucideIcon name="message-square-check" :size="20" class="text-accent opacity-50" />
                </template>
              </v-text-field>
            </v-col>
          </v-row>
      ...
      <v-card-actions class="pa-8 modal-footer-solid modal-footer-sticky">
        ...
        <v-spacer />
        <v-btn variant="flat" size="large" class="px-12 font-weight-black premium-button-highlight action-btn-unified h-56" :disabled="!formValid || caseBlockStatus.is_blocked" :loading="saving" @click="handleSave">{{ isEditing ? 'تحديث الموعد' : 'تأكيد الجدولة' }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
watch(() => editItem.value.case_id, async (newVal) => {
...
const arabicFilter = (item: any, query: string): boolean => {
  ...
  return item.display.toLowerCase().includes(q) || item.case_number?.toLowerCase().includes(q) || item.client_name?.toLowerCase().includes(q)
}
</script>
