<template>
  <div>
    ...
    <v-card v-for="session in safeArray(sessions)" v-else :key="session.id" elevation="0" class="glass-card mb-3 rounded-xl overflow-hidden session-mobile-card">
      <div class="session-card-header pa-3 d-flex align-center justify-space-between">
        ...
        <v-chip :color="getStatusColor(session.status)" size="x-small" variant="flat" class="font-weight-black rounded-md">{{ session.status }}</v-chip>
      </div>
        ...
        <div class="d-flex align-start justify-space-between">
          <div class="flex-grow-1">
            <div class="d-flex align-center gap-1 mb-2">
              ...
              <span class="text-caption font-weight-black text-accent">{{ session.case_number || 'بدون رقم قضية' }}</span>
            </div>
            <div class="d-flex align-center gap-1 mb-2">
              <LucideIcon name="user" :size="14" class="text-gold opacity-60" />
              <span class="text-caption font-weight-black">{{ session.client_name || 'بدون موكل' }}</span>
            </div>
          ...
          <div class="d-flex flex-column gap-1 ms-2">
            <v-btn icon variant="tonal" color="accent" size="small" class="rounded-lg" @click="$emit('open-session-room', session)">
              <LucideIcon name="swords" :size="16" />
            </v-btn>
            <v-btn icon variant="tonal" color="gold" size="small" class="rounded-lg" @click="$emit('edit', session)">
              <LucideIcon name="edit-3" :size="16" />
            </v-btn>
            <v-btn icon variant="tonal" color="error" size="small" class="rounded-lg" @click="$emit('delete', session)">
    ...
    <div class="d-flex justify-center mt-4">
      <v-pagination v-model="page" :length="Math.ceil(totalSessions / itemsPerPage)" :total-visible="3" density="compact" color="accent" />
    </div>
  </div>
</template>
