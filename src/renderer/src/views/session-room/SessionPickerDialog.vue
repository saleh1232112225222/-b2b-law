<template>
  <v-dialog v-model="showVal" max-width="760">
    <v-card class="rounded-xl glass-card border shadow-premium overflow-hidden glass-card">
      ...
      <v-card-text class="pa-6 bg-noir-surface glass-card">
        <v-alert
          ...
          text="لا توجد جلسات اليوم أو الغد."
        >
        ...
        <v-list v-else density="comfortable" class="bg-transparent ga-3">
          <v-list-item
            v-for="s in pickOptions"
            :key="s.id"
            class="rounded-xl glass-card border mb-2 premium-hover"
            @click="choose(s)"
          >
            <template #prepend>
              <div class="pa-2 rounded-lg bg-primary-alpha me-3">
                <LucideIcon name="calendar" :size="20" class="text-primary" />
              </div>
            </template>
            <v-list-item-title class="font-weight-black text-primary"
              >{{ s.case_number || 'جلسة' }} — {{ formatSessionDate(s) }}</v-list-item-title
            >
            <v-list-item-subtitle class="text-caption font-weight-bold text-text-muted">{{
              s.client_name || '—'
            }}</v-list-item-subtitle>
            <template #append
              ><LucideIcon name="arrow-left" :size="20" class="text-primary"
            /></template>
          </v-list-item>
        </v-list>
      </v-card-text>
