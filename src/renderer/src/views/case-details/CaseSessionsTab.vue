<template>
  <div>
    <v-alert v-if="blocked" type="error" variant="tonal" class="glass-card mb-6">
      <template #prepend><LucideIcon name="shield-alert" :size="24" class="text-error me-3" /></template>
    ...
    <v-btn color="accent" variant="flat" size="small" class="mb-6 rounded-lg font-weight-black text-primary-dark" :disabled="blocked" @click="$emit('add')">
    ...
    <v-table class="premium-table mt-4">
      ...
      <tbody>
        <tr v-if="sessions.length === 0">
          <td colspan="5" class="text-center pa-8 text-gold opacity-50 italic">لا توجد جلسات مسجلة</td>
        </tr>
        <tr v-for="s in sessions" :key="s.id" class="premium-hover">
          <td>
            <div class="d-flex flex-column py-1">
              <span class="text-subtitle-2 font-weight-black text-visible-high">{{ s.date }} مـ</span>
              <v-chip size="x-small" color="accent" variant="tonal" class="mt-1 font-weight-black" style="max-width: fit-content">
          ...
          <td>
            <v-btn v-if="s.meeting_link" color="accent" size="x-small" variant="tonal" class="rounded-lg px-3 font-weight-black" @click="openLink(s.meeting_link)">
...
const openLink = (url: string): void => { window.open(url, '_blank') }
</script>
