<template>
  <v-row>
    <v-col cols="12" md="6">
      <v-list class="pa-0">
        <v-list-item>
          <template #prepend><LucideIcon name="landmark" :size="20" class="text-gold me-3" /></template>
          <v-list-item-title class="font-weight-black text-visible-high">المحكمة والدائرة</v-list-item-title>
          <v-list-item-subtitle class="text-gold">{{ caseItem.court || 'غير محدد' }} - {{ caseItem.circuit || 'بدون دائرة' }}</v-list-item-subtitle>
        </v-list-item>
        <v-list-item>
          <template #prepend><LucideIcon name="calendar" :size="20" class="text-gold me-3" /></template>
          <v-list-item-title class="font-weight-black text-primary">تاريخ القيد</v-list-item-title>
          <v-list-item-subtitle class="text-primary font-weight-bold">{{ caseItem.registration_date }} مـ | {{ caseItem.registration_date_hijri }} هـ</v-list-item-subtitle>
        </v-list-item>
        ...
        <v-list-item v-for="party in parties" :key="party.id" class="mb-2 glass-card mx-2 border-0">
          ...
          <v-list-item-title class="font-weight-black text-visible-high text-body-2">
            {{ party.name }}
            <v-chip size="x-small" color="primary" variant="flat" class="ms-1 px-2 font-weight-black">{{ party.party_type === 'client' ? 'موكل' : 'خصم' }}</v-chip>
          </v-list-item-title>
          <v-list-item-subtitle class="text-caption text-primary font-weight-bold">
            {{ party.role }}<span v-if="party.phone" class="ms-2"><LucideIcon name="phone" :size="10" class="me-1" /> {{ party.phone }}</span>
    ...
    <v-col cols="12" md="6">
      <v-card elevation="0" class="glass-card pa-6 h-100">
        <div class="d-flex justify-space-between align-center mb-6">
          ...
          <v-chip color="success" size="large" variant="flat" class="font-weight-black">{{ caseItem.status }}</v-chip>
        </div>
        <v-row dense>
          <v-col cols="6">
            <div class="text-caption grey--text mb-1">المرحلة الحالية</div>
            <v-chip size="small" variant="tonal" color="indigo">{{ caseItem.phase || 'غير محددة' }}</v-chip>
          </v-col>
          <v-col cols="6">
            <div class="text-caption grey--text mb-1">صفة الموكل</div>
            <v-chip size="small" variant="tonal" color="teal">{{ caseItem.client_role || 'غير محددة' }}</v-chip>
          </v-col>
        </v-row>
