<template>
  <v-card elevation="0" class="glass-card overflow-hidden">
    <div class="glass-panel px-6 pt-2">
      <v-tabs :model-value="tab" @update:model-value="$emit('update:tab', $event)" color="accent" align-tabs="start" class="premium-tabs">
    ...
    <v-card-text class="pa-8">
      <v-alert v-if="!can('view_contracts')" type="error" variant="flat" class="rounded-lg mb-6 border-dashed">
      ...
      <div v-else>
        <v-row dense class="mb-8 align-center">
          <v-col cols="12" md="4">
            <v-text-field :model-value="search" @update:model-value="$emit('update:search', $event)" variant="outlined" density="comfortable" placeholder="بحث برقم العقد، العنوان، أو اسم الطرف..." hide-details class="glass-input" clearable>
              <template #prepend-inner><LucideIcon name="search" :size="20" class="text-gold opacity-50" /></template>
            </v-text-field>
          </v-col>
          <v-spacer />
          <v-col cols="auto">
            <v-btn variant="text" color="gold" class="rounded-lg opacity-70" :loading="loading" @click="$emit('reload')">
          ...
          <tbody>
            <tr v-if="filtered.length === 0">
              <td colspan="6" class="text-center pa-8">
                ...
                <div class="text-h6 text-gold opacity-40">لا توجد عقود {{ tab === 'employment' ? 'توظيف' : 'أتعاب' }} مطابقة</div>
              </td>
            </tr>
            <tr v-for="c in filtered" :key="c.id" class="premium-hover-row">
              <td>
                ...
                <div class="text-tiny text-gold opacity-50 font-weight-black">{{ c.contract_no || '—' }}</div>
              </td>
              <td>
                <v-chip size="x-small" :color="c.contract_type === 'employment' ? 'accent' : 'info'" variant="flat" class="font-weight-black rounded-md">
                  {{ c.contract_type === 'employment' ? 'توظيف' : 'أتعاب' }}
                </v-chip>
              </td>
              <td>
                <v-chip size="x-small" :color="c.status === 'approved' ? 'success' : c.status === 'archived' ? 'grey' : 'warning'" variant="flat" class="font-weight-black rounded-md">
                  {{ c.status === 'approved' ? 'معتمد' : c.status === 'archived' ? 'مؤرشف' : 'قيد الانتظار' }}
                </v-chip>
              </td>
              ...
              <td>
                <div class="d-flex align-center gap-2">
                  <v-btn icon size="small" variant="tonal" color="accent" class="rounded-lg" @click="$emit('view', c.id)">
                    <LucideIcon name="eye" :size="16" />
                    <v-tooltip activator="parent" location="top">معاينة العقد</v-tooltip>
                  </v-btn>
                  <v-btn v-if="c.status === 'pending'" icon size="small" variant="tonal" color="success" class="rounded-lg" @click="$emit('approve', c.id)">
                    <LucideIcon name="check" :size="16" />
                    <v-tooltip activator="parent" location="top">اعتماد العقد</v-tooltip>
                  </v-btn>
                  <v-btn v-if="c.status !== 'archived'" icon size="small" variant="tonal" color="grey" class="rounded-lg" @click="$emit('archive', c.id)">
