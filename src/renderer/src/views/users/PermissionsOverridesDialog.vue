    ...
    <v-card class="glass-card border-gold border-opacity-30 border-2 overflow-hidden">
      ...
      <v-card-text class="pa-8">
        <div class="glass-panel-light pa-4 rounded-xl mb-6 border border-gold border-opacity-20">
          ...
          <div class="text-body-2 text-white opacity-60">يتم تطبيق صلاحيات الدور الوظيفي بشكل آلي، ويمكنك هنا إضافة استثناءات (سماح/منع) محددة لهذا المستخدم فقط.</div>
        </div>
        <div class="mb-6 d-flex gap-3">
          <v-btn color="success" variant="flat" class="rounded-lg font-weight-black px-6" @click="handleBulkSet(true)">
            <LucideIcon name="check-check" :size="18" class="me-2" /> سماح للكل
          </v-btn>
          <v-btn color="error" variant="flat" class="rounded-lg font-weight-black px-6" @click="handleBulkSet(false)">
        ...
        <v-table density="compact" class="glass-table border border-gold border-opacity-10 rounded-xl overflow-hidden">
          ...
          <tbody>
            <tr v-for="p in safeArray(permissions)" :key="p.permission_key" class="premium-hover-row">
              <td class="text-accent font-weight-bold">{{ p.module_key || '-' }}</td>
              <td>
                <div class="text-white font-weight-black">{{ p.permission_name || '-' }}</div>
                <div class="text-caption text-gold opacity-40 font-mono">{{ p.permission_key || '-' }}</div>
              </td>
              <td class="text-center">
                <div class="d-flex align-center justify-center gap-2">
                  <v-btn size="small" :variant="getOverrideState(p.permission_key) === true ? 'flat' : 'tonal'" color="success" class="font-weight-black" min-width="80" @click="setOverride(p.permission_key, true)">سماح</v-btn>
                  <v-btn size="small" :variant="getOverrideState(p.permission_key) === false ? 'flat' : 'tonal'" color="error" class="font-weight-black" min-width="80" @click="setOverride(p.permission_key, false)">منع</v-btn>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <v-card-actions class="pa-6 glass-panel-light border-t border-gold border-opacity-10">
        <v-spacer />
        <v-btn color="accent" variant="flat" class="px-10 font-weight-black text-ebony rounded-lg" @click="$emit('update:show', false)">إغلاق وتاكيد</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
watch(() => props.show, async (val) => {
  if (val && props.selectedUserId) {
    try {
      ...
      userOverrides.value = safeArray(await (window as any).api.users.getOverrides(props.selectedUserId))
    } catch (e) {
...
const setOverride = async (permissionKey: string, isAllowed: boolean): Promise<void> => {
  ...
  try {
    await (window as any).api.users.setPermissionOverride(props.selectedUserId, permissionKey, isAllowed)
    userOverrides.value = safeArray(await (window as any).api.users.getOverrides(props.selectedUserId))
    emit('done')
  } catch (e: unknown) {
...
const handleBulkSet = async (isAllowed: boolean): Promise<void> => {
  ...
  try {
    await (window as any).api.users.setBulkPermissionOverrides(props.selectedUserId, isAllowed, keys)
    userOverrides.value = safeArray(await (window as any).api.users.getOverrides(props.selectedUserId))
    emit('done')
  } catch (e: unknown) {
